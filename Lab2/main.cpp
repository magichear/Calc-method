...
public:
    /**
     * @param n_nodes 插值函数的阶数
     * @param nodeTy 插值节点生成方法，0: 均匀取点，1: 切比雪夫节点
     * @param func 原始函数
     * @param nodeFunc 插值节点生成方法
     */
    LagrangeErrComparator(int n_nodes, int nodeTy, std::function<long double(long double)> func, std::function<long double(int)> nodeFunc = nullptr) 
        : n(n_nodes), originFunc(func), nodeFunc(nodeFunc) {
        __init(nodeTy);
    }
